...
func main() {
	...
	default:
		slog.Error("unknown worker type", "type", "certificates")
		os.Exit(1)
	}

	slog.Info("certificates worker starting")
	if err := w.Start(ctx); err != nil {
		slog.Error("certificates worker error", "error", err)
		os.Exit(1)
	}
	slog.Info("certificates worker stopped")
}
