...
func main() {
	...
	default:
		slog.Error("unknown worker type", "type", "databases")
		os.Exit(1)
	}

	slog.Info("databases worker starting")
	if err := w.Start(ctx); err != nil {
		slog.Error("databases worker error", "error", err)
		os.Exit(1)
	}
	slog.Info("databases worker stopped")
}
