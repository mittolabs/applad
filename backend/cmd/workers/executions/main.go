...
func main() {
	...
	default:
		slog.Error("unknown worker type", "type", "executions")
		os.Exit(1)
	}

	slog.Info("executions worker starting")
	if err := w.Start(ctx); err != nil {
		slog.Error("executions worker error", "error", err)
		os.Exit(1)
	}
	slog.Info("executions worker stopped")
}
