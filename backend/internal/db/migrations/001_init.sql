...
CREATE TRIGGER set_updated_at BEFORE UPDATE ON billing_plans
    FOR EACH ROW EXECUTE FUNCTION applad_set_updated_at();
...
CREATE INDEX IF NOT EXISTS idx_bs_project ON billing_subscriptions (project_id);
CREATE INDEX IF NOT EXISTS idx_bs_plan    ON billing_subscriptions (plan_id);
...
CREATE TABLE IF NOT EXISTS billing_events (
    ...
    metadata   JSONB,
    created_at TIMESTAMPTZ  NOT NULL DEFAULT NOW(),
    CONSTRAINT fk_be_project FOREIGN KEY (project_id) REFERENCES projects(id) ON DELETE CASCADE
);
CREATE INDEX IF NOT EXISTS idx_be_project ON billing_events (project_id);
CREATE INDEX IF NOT EXISTS idx_be_type    ON billing_events (project_id, event_type);
CREATE INDEX IF NOT EXISTS idx_be_created ON billing_events (project_id, created_at);
