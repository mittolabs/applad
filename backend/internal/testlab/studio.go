...
// writeGeneratedProject lays a recording down as a runnable Playwright project
// at the path the runner reads uploaded sources from, so a saved flow needs no
// upload step of its own.
func writeGeneratedProject(suiteID, flowName, spec string) error {
	dir, err := os.MkdirTemp("", "applad-flow-*")
	if err != nil {
		return err
	}
	...
	files := map[string]string{
		...
		"playwright.config.js": `// Generated by Applad. Every test is recorded, so a failure can be watched.
module.exports = {
  testDir: './tests',
...
`,
		filepath.Join("tests", "recorded.spec.js"): spec,
	}
	...
	return tarGzDir(dir, deploy.SourceArchivePath(suiteID))
}
