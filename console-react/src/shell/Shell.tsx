...
export function Shell() {
  ...
  const aiEnabled = useExperimentsStore((s) => s.flags.aiChat);
  ...
  const aiFab = aiEnabled && !aiOpen && (
    <button
      onClick={() => setAiOpen(true)}
      className="fixed bottom-20 right-6 z-40 flex h-11 w-11 items-center justify-center rounded-full bg-[var(--color-accent)] text-white shadow-[0_8px_32px_var(--shadow)] transition-transform hover:scale-105 md:bottom-6"
  ...
  if (isMobile) {
    return (
      <div className="flex h-screen flex-col overflow-hidden bg-background">
        ...
        {aiEnabled && aiOpen && <AiChatPanel onClose={() => setAiOpen(false)} />}
        {aiFab}
      </div>
    );
  }
