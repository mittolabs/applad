...
func main() {
	...
	default:
		slog.Error("unknown worker type", "type", "webhooks")
		os.Exit(1)
	}

	slog.Info("webhooks worker starting")
	if err := w.Start(ctx); err != nil {
		slog.Error("webhooks worker error", "error", err)
		os.Exit(1)
	}
	slog.Info("webhooks worker stopped")
}
