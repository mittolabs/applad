...
func main() {
	...
	default:
		slog.Error("unknown worker type", "type", "migrations")
		os.Exit(1)
	}

	slog.Info("migrations worker starting")
	if err := w.Start(ctx); err != nil {
		slog.Error("migrations worker error", "error", err)
		os.Exit(1)
	}
	slog.Info("migrations worker stopped")
}
